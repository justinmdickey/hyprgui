...
def test_is_linked_ignores_comments_and_mentions(tmp_path):
    _redirect_hypr_dir(tmp_path)
    # A commented-out dofile and a prose mention of the filename must NOT count.
    (tmp_path / "hyprland.lua").write_text(
        "-- consider: dofile(os.getenv('HOME') .. '/.config/hypr/hyprgui.lua')\n"
        "-- hyprgui.lua is generated by Hyprgui\n"
        "hl.config({ general = { gaps_in = 5 } })\n"
    )
    assert not lua_writer.is_linked()


def test_is_linked_accepts_handedited_variants(tmp_path):
    _redirect_hypr_dir(tmp_path)
    cases = [
        'dofile(os.getenv("HOME") .. "/.config/hypr/hyprgui.lua")\n',
        "dofile('/home/me/.config/hypr/hyprgui.lua')\n",
        '  dofile ( os.getenv("HOME") .. "/.config/hypr/hyprgui.lua" )  -- trailing comment\n',
        'require("hyprgui")\n',
        'DOFILE("hyprgui.lua")\n',  # case-insensitive, just in case
    ]
    for src in cases:
        (tmp_path / "hyprland.lua").write_text(src)
        assert lua_writer.is_linked(), f"should detect link in: {src!r}"


def test_link_line_is_plain_lua_not_python_fstring(_tmp_path=None):
    # Regression: it was an f-string with no interpolation, which misled review.
    # The os.getenv(...) is Lua source meant for Lua to evaluate, not Python.
    assert "os.getenv" in lua_writer._LINK_LINE
    assert "{" not in lua_writer._LINK_LINE  # no Python interpolation
